...
#AUDIO_SAMPLE_RATES="48000,88200,96000,176400,192000"
...
case "$1" in
    start)
            ...
            USB_ETH=$(grep -w "UsbEth" $RIVA_HOME/config.toml | cut -f2 -d'=' | sed 's/ //g')
            USB_IP=$(grep -w "UsbIPaddr" $RIVA_HOME/config.toml | cut -f2 -d'=' | sed -E 's/ |"//g')
            USB_MASK=$(grep -w "UsbNMask" $RIVA_HOME/config.toml | cut -f2 -d'=' | sed -E 's/ |"//g')
            [[ -z "$USB_IP" ]] && USB_IP="192.168.0.1"
            [[ -z "$USB_MASK" ]] && USB_MASK="255.255.255.0"
            ...
            echo "RIVA" > strings/0x409/manufacturer
            echo "RIVA USB" > strings/0x409/product
